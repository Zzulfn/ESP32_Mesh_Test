...
static uint16_t node_net_idx =0x00; //ESP_BLE_MESH_KEY_UNUSED;
static uint16_t node_app_idx = 0x00;//ESP_BLE_MESH_KEY_UNUSED;
static int remote_onoff = LED_OFF;
static uint8_t prov_complete_mark = 0;
/* The remote node address shall be input through UART1, see board.c */
uint16_t remote_addr = ESP_BLE_MESH_ADDR_ALL_NODES;//ESP_BLE_MESH_ADDR_UNASSIGNED;
...
static void esp_ble_mesh_prov_cb(esp_ble_mesh_prov_cb_event_t event, esp_ble_mesh_prov_cb_param_t *param)
{
                                  ...
                                  esp_ble_mesh_model_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_MODEL_OPERATION_EVT: {
        ...
        case ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET: {
            ...
            ESP_LOGI(TAG, "Dick esp_ble_mesh_set_msg_common");
			#if 0
        ...
        case ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET_UNACK: {
			 ...
			 ESP_LOGI(TAG, "Dick ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET_UNACK\n");
            esp_ble_mesh_generic_client_set_state_t set_state = {0};
...
{
    int err;
	unsigned char Remote_add[MaxAddrNum] = {0};
	unsigned char Ledstation =0;
	int RemoteAdd = 0;
	int i =0;
	unsigned char Data  = 0;
	unsigned char sendnum = 0;
	unsigned int  sendnum2 = 0;
	//esp_ble_mesh_client_common_param_t common = {0}; //used to test BLE_MESH
	...
	while(1){
		...
		if(prov_complete_mark)
		{    
				#if 1
				vTaskDelay(200); 
				ESP_LOGI(TAG, "Dick start send contorl data to element :ALl\n");
				esp_ble_mesh_set_msg_common(&common, &set_state, onoff_client.model,
										BLE_MESH_MODEL_OP_GEN_ONOFF_SET_UNACK, Data);
				
			  //  common.ctx.addr =0xFFFF;
			   ESP_LOGI(TAG, "Dick esp_ble_mesh_generic_client_set_state common.ctx.addr:%d \n",common.ctx.addr);
				err = esp_ble_mesh_generic_client_set_state(&common, &set_state);
				if (err != ESP_OK) {
					ESP_LOGE(TAG, "%s: Generic OnOff Set failed", __func__);
					return;
				...
				#if 0
				vTaskDelay(1000); 
				ESP_LOGI(TAG, "Dick start send contorl data to element :7\n");
				esp_ble_mesh_set_msg_common(&common, &set_state, onoff_client.model,
										ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET, Data);
				
			    common.ctx.addr =0x0B;
				err = esp_ble_mesh_generic_client_set_state(&common, &set_state);
				if (err != ESP_OK) {
					ESP_LOGE(TAG, "%s: Generic OnOff Set failed", __func__);
					return;
				...
				#endif
				ESP_LOGI(TAG, "Dick sendnum :0X%04x      SendData：0X%04x",sendnum2,Data);
				Data++;
				sendnum++;
				sendnum2++;
			}
